...
def get_config_file_settings():
    try:
        ...
        global remediateTUR
        global remediateTUR_flag
        remediateTUR = readConfigFile(configFile, "remediateTUR", "int")
        printLog("remediateTUR on or off: {}.".format(remediateTUR))
        try:
            remediateTUR_flag = True if remediateTUR == 1 else False
        except Exception as err:
            temp_msg = f'Attemtped to set remediateTUR_flag boolean; got error:\n{err}'
            printLog(temp_msg)
            error_and_exit(temp_msg)
        printLog("remediateTUR_flag set to {}.".format(remediateTUR_flag))
...
def get_dut_template_data(template_filepath=''):
    ...
    print(f"Loading template file at path: {template_filepath}")
    ...
    global amplifier
    temp_value = readConfigFile(templateFile, "amplifier", sFunc="int")
    amplifier = sanitize_variable(temp_value, default_response=0, specified_class='int',
                                     eval_operation='at least',
                                     eval_threshold=0)
    printLog(f"amplifier - Template File Value: {temp_value}, Sanitized Value: {amplifier}", console=False)
    
    
    global ampGain
    temp_value = readConfigFile(templateFile, "ampGain", sFunc="int")
    ampGain = sanitize_variable(temp_value, default_response=0, specified_class='int',
                                     eval_operation='at least',
                                     eval_threshold=0)
    printLog(f"ampGain - Template File Value: {temp_value}, Sanitized Value: {ampGain}", console=False)
    
    global ampSettlingSampleQty
    temp_value = readConfigFile(templateFile, "ampSettlingSampleQty", sFunc="int")
    ampSettlingSampleQty = sanitize_variable(temp_value, default_response=10, specified_class='int',
                                     eval_operation='at least',
                                     eval_threshold=0)
    printLog(f"ampSettlingSampleQty - Template File Value: {temp_value}, Sanitized Value: {ampSettlingSampleQty}", console=False)
    
    global ampSettleGoal
    temp_value = readConfigFile(templateFile, "ampSettleGoal", sFunc="int")
    ampSettleGoal = sanitize_variable(temp_value, default_response=10, specified_class='int',
                                     eval_operation='at least',
                                     eval_threshold=0)
    printLog(f"ampSettleGoal - Template File Value: {temp_value}, Sanitized Value: {ampSettleGoal}", console=False)
    ...
    global pmRangeAuto
    pmRangeAuto = readConfigFile(pm_driver, "pmRangeAuto", errorVal="driver cmd not found")
    printLog(f"pmRangeAuto: {pmRangeAuto}", console=False)
    
    global pmRangeSet
    pmRangeSet = readConfigFile(pm_driver, "pmRangeSet", errorVal="driver cmd not found")
    printLog(f"pmRangeSet: {pmRangeSet}", console=False)
    
    global pmRangeRead
    pmRangeRead = readConfigFile(pm_driver, "pmRangeRead", errorVal="driver cmd not found")
    printLog(f"pmRangeRead: {pmRangeRead}", console=False)
    ...
    # Get Zero Set instructions, if applicable
    ...
    # zeroSetQtyOfSamples = 3
    # zeroSetQtyOfRuns = 10
    # zeroSetToleranceWatts = 0.000000025
    # 
    # pmConfigZS = INIT:CONT OFF; UNIT:POW DBM; CAL:TYPE INT; FREQ 50MHz
    # pmTrigMeas = INIT:IMM
    # pmAvgQuery = SENS1:AVER:COUN:AUTO?
    # pmAutoAvgOn = AVER:COUN:AUTO ON
    
    global performZeroSetCal
    performZeroSetCal = False
    temp_value = readConfigFile(templateFile, "performZeroSetCal")
    temp_value = temp_value.lower()
        
    if "yes" in temp_value:
        performZeroSetCal = True
    else:
        performZeroSetCal = False
    
    printLog(f"performZeroSetCal - Template File Value: {temp_value}, Sanitized Boolean Value: {performZeroSetCal}", console=False)
    
    
    global appliedPowerFlagLevel
    temp_value = readConfigFile(templateFile, "biasMsmtQty")
    appliedPowerFlagLevel = sanitize_variable(temp_value, default_response=-30, specified_class='int',
                                    eval_operation='at least',
                                    eval_threshold= -999)
    printLog(f"appliedPowerFlagLevel - Template File Value: {temp_value}, Sanitized Value: {appliedPowerFlagLevel}", console=False)
    
    global zeroSetQtyOfSamples
    zeroSetQtyOfSamples = ""
    temp_value = readConfigFile(templateFile, "zeroSetQtyOfSamples")
    zeroSetQtyOfSamples = sanitize_variable(temp_value, default_response=10, specified_class='int',
                                    eval_operation='at least',
                                    eval_threshold=1)
    printLog(f"zeroSetQtyOfSamples - Template File Value: {temp_value}, Sanitized Value: {zeroSetQtyOfSamples}", console=False)
    
    global zeroSetQtyOfRuns
    zeroSetQtyOfRuns = ""
    temp_value = readConfigFile(templateFile, "zeroSetQtyOfRuns")
    zeroSetQtyOfRuns = sanitize_variable(temp_value, default_response=10, specified_class='int',
                                    eval_operation='at least',
                                    eval_threshold=1)
    printLog(f"zeroSetQtyOfRuns - Template File Value: {temp_value}, Sanitized Value: {zeroSetQtyOfRuns}", console=False)
    
    global intZeroSetToleranceWatts
    intZeroSetToleranceWatts = ""
    temp_value = readConfigFile(templateFile, "intZeroSetToleranceWatts")
    intZeroSetToleranceWatts = sanitize_variable(temp_value, default_response=10, specified_class='float',
                                    eval_operation='at least',
                                    eval_threshold=0)
    printLog(f"intZeroSetToleranceWatts - Template File Value: {temp_value}, Sanitized Value: {intZeroSetToleranceWatts}", console=False)
    
    global extZeroSetToleranceWatts
    extZeroSetToleranceWatts = ""
    temp_value = readConfigFile(templateFile, "extZeroSetToleranceWatts")
    extZeroSetToleranceWatts = sanitize_variable(temp_value, default_response=10, specified_class='float',
                                    eval_operation='at least',
                                    eval_threshold=0)
    printLog(f"extZeroSetToleranceWatts - Template File Value: {temp_value}, Sanitized Value: {extZeroSetToleranceWatts}", console=False)
    
    if intZeroSetToleranceWatts == 10 and extZeroSetToleranceWatts == 10:
        # Look for the older single range zero set keyword "zeroSetToleranceWatts"
        intZeroSetToleranceWatts = ""
        temp_value = readConfigFile(templateFile, "zeroSetToleranceWatts")
        intZeroSetToleranceWatts = sanitize_variable(temp_value, default_response=10, specified_class='float',
                                        eval_operation='at least',
                                        eval_threshold=0)
        printLog(f"intZeroSetToleranceWatts - Template File Value: {temp_value}, Sanitized Value: {intZeroSetToleranceWatts}", console=False)
    ...
    printLog(f"pmConfigZS: {pmConfigZS}", console=False)
    if ";" in pmConfigZS:
        pmConfigZS = pmConfigZS.split(";")
        for index, item in enumerate(pmConfigZS):
            item = item.strip()
            pmConfigZS[index] = item
            
    printLog(f"pmConfigZS: {pmConfigZS}", console=False)
    
    global pmExtZero
    pmExtZero = readConfigFile(pm_driver, "pmExtZero")
    printLog(f"pmExtZero: {pmExtZero}", console=False)
    global pmIntZero
    pmIntZero = readConfigFile(pm_driver, "pmIntZero")
    printLog(f"pmIntZero: {pmIntZero}", console=False)
...
def level_generator_and_power_meter(target_level, leveling_tol=0.02, settling_time=5, safe=True, max_output=20, retry_max=50, ampUsed=0, approxGain=0):
    ...
    def fine_level_generator_routine(target_level, sig_gen_set_level, loop_interval=0.75):
        ...
        while leveling == True:
            ...
            # if sig_gen_set_level > target_level:
            #    temp_str = f"Warning! sig_gen_set_level is {sig_gen_set_level} dBm, which is greater than the target level ({target_level} dBm).\nEnsure the power sensor will not be damaged by the new level before proceeding..."
            #    input(temp_str)
            ...
            time.sleep(loop_interval) # Make sure the sensor has a chance to respond
            
            pm_level = queryVisa(pmeter_resource, pmRead) # Ensure the PM definately measures the latest applied power level
            pm_level = queryVisa(pmeter_resource, pmRead)
            pm_level = queryVisa(pmeter_resource, pmRead)
            pm_level = sanitize_variable(pm_level, specified_class='float', default_response=999_999_999)
            # printLog(pm_level)
            ...
            # Try to make sure we don't blow up the power sensor with all this shoddy leveling logic
            pm_diff_sig_gen_set_level = sig_gen_set_level - pm_level
            future_pm_level_at_next_iteration =  sig_gen_set_level - pm_diff_sig_gen_set_level
            
            if future_pm_level_at_next_iteration > target_level:
                temp_float = future_pm_level_at_next_iteration - target_level
                sig_gen_set_level = sig_gen_set_level - temp_float
    ...
    def settleAmp(tol, sampleQty):
        print("")
        print("====================================")
        print("Settling amplifier...")
        print("====================================\n")
        dB_deviation = 100
        results_list = []
        achievedGoalCounter = 0
        while achievedGoalCounter < 1:
            # Get samples
            avg_msmt, stdev_pct = sample_power_meter(sampleQty, interval=0.1, meter_unit="dBm", status_bar=True)
            
            msmt_mw = dBm_mW(avg_msmt)
            
            pct_adder = msmt_mw / 100 * stdev_pct
            
            msmt_lower = msmt_mw - pct_adder
            msmt_upper = msmt_mw + pct_adder
            
            dBm_lower = mW_dBm(msmt_lower) 
            dBm_upper = mW_dBm(msmt_upper)
            
            dB_deviation = abs(dBm_upper - dBm_lower) / 2
            
            if dB_deviation <= tol:
                achievedGoalCounter += 1
            else:
                achievedGoalCounter =0
                
            string = f"Measured: {avg_msmt:.2f} dBm, Deviation +/- {stdev_pct:.2f}% ({dB_deviation:.3f} dB)"
            ...
            for string in results_list:
                print(string)
            print(f"Goal: {tol:.2f} dB")
        
        print("\n====================================")
        print("Achieved Amplifier Settling Goal!")
        print("====================================\n")
            
    
    def testGeneratorStability():
        # Thought about building this out for troubleshooting generator stability issues, but ultimately 
        # opted not to.

        def msrStability():
            avg_msmt, stdev_pct = sample_power_meter(100, interval=0.1, meter_unit="dBm", status_bar=True)
            ...
            stdev_dB = abs(dBm_upper - dBm_lower) / 2
            
            return stdev_pct, stdev_dB
                
        print("")
        print("====================================")
        print("Testing Generator Stability...")
        print("====================================\n")
        
        
        
        freqList = [10000000, 50000000, 500000000, 1000000000, 2000000000, 3000000000, 4000000000, 5000000000, 6000000000, 7000000000, 8000000000, 9000000000, 10000000000, 20000000000, 30000000000, 40000000000, 50000000000]
        powerList = [-10, -5, -2, 0, 2, 5, 10]
        
        for frequency in freqList:
            response = writeVisa(generator_resource, sGenFreqSet.replace('<val>', f"{frequency}"))
            print(f"Tesing {frequency} Hz")
            time.sleep(5)
            
            for power in powerList:
                response = writeVisa(generator_resource, sGenPowSet.replace('<val>', f"{power}"))
                time.sleep(5)
                
                stdev_pct, stdev_dB = msrStability
    ...
    while abs_delta > leveling_tol:
        ...
        pm_level = queryVisa(pmeter_resource, pmRead)
        pm_level = sanitize_variable(pm_level, specified_class='float', default_response=999_999_999)
        print(f"PM Measured: {pm_level:.2f} dBm")

        delta = target_level - pm_level
        if ampUsed == 1:        # Force the leveling process to go really slow, when using an amp
        ...
        if abs_delta <= leveling_tol and ampUsed == 1:  # Once leveling_tol is reached, settle the amplifier (if used)
            settleAmp(ampSettleGoal, ampSettlingSampleQty)
...
class GuiProgramWindow:
    ...
    def __init__(self, windowTitle, ConsoleStringList=[], consoleQuantityOfLines=20, inputString=''):
        ...
        self.performZS = False
        self.zs_eval_type = "17025"
    ...
    def _thread_function(self):
        ...
        layout = [[self.sg.Menu(menu_def, tearoff=False)],
                  ...
                  [self.sg.Text('_' * lineLength)],
                  [self.sg.Checkbox('Perform Zero Set Calibration', default=self.performZS, key="-zs_cal-"),
                   self.sg.Button('Zero Set Test Info', **resource_button, key='-zs_info_btn-', disabled=True)],
                  [self.sg.Text('_' * lineLength)],
    ...
    def load_dut_template(self):
        ...
        if len(self.dut_template_filepath) == 0:
            ...
            print(f"Template loaded successfully? {successful_template_load}")
            ...
            self.window['-zs_cal-'].update(performZeroSetCal)
            if performZeroSetCal:
                self.window['-zs_info_btn-'].update(disabled=False)
    ...
    def about_zs_test(self):
        temp_str = f'So I took the block of code for the zero set test out of the old linearity program (the one that ' \
                   f'used the FLU/96270A), and I dropped it into this program, and I connected whatever variables ' \
                   f'and functions the code depended on. I havene\'t had time to extensively test all the ' \
                   f'possible logical errors that might exist. The calibration routine works and is accurate, that\'s ' \
                   f'about all I can say about it, right now. Maybe one day I\'ll do a minor overhaul so that SCPI ' \
                   f'command drivers can be incorporated more flexibly. For now, this is what it is. Good luck.'

        temp_int = yes_no_other_popup(temp_str, other_str_text='  I Don\'t Care ', btn_focus=1, window_title='About Zero Set Testing',
                           lineLength=120)
        print(f'tempint: >{temp_int}<')
        
        if temp_int == 1:
            msg_box_simple("Good answer.")
        if temp_int == 0:
            msg_box_simple("Too bad.")
        if temp_int == -1:
            msg_box_simple("I don\'t care either. Welcome to the club.")
...
def sample_power_meter(qty, interval=1, meter_unit="dBm", status_bar=True):
    ...
    try:
        ...
        for number in range(qty):
            ...
            # writeVisa(pmeter_resource, pmTrigMeas)
            time.sleep(interval)
            pm_level = queryVisa(pmeter_resource, pmRead)
            # printLog(pm_level)
            pm_level = sanitize_variable(pm_level, specified_class='float', default_response=999_999_999)
        ...
        # Ran into an issue where, if the PM measured overload, then the value appended to sdev_list could be larger than the CPU
        # can store into one variable, via Python's methods. In such instances then the appended value simply becomes the
        # maximum value that Python can store in a float
        printLog(f"Eror during sample_power_meter routine: {error}", console=False)
        sdev_List.append(sys.float_info.max)
...
def sample_power_meter_avg_of_avgs(sample_qty, meter_unit, lin_step, interval=1, qty_of_msmt_runs = 10, turValue=0):
    if turValue == 0:
        turValue = "null"
    else:
        
        turValue = f"{turValue:.3f}:1"
    
    avg_list = []
    sdev_List = []
    
    for number in range(qty_of_msmt_runs):
        clear()
        print(f"The system detected that the TUR was really bad for this step!\nThe system is attempting to improve the TUR by taking more measurements.\n")
        print(f"Last TUR was {turValue}")
        print(f"Measuring Linearity Step: {lin_step} dBm")
        print(f"Getting Sample {number + 1} of {qty_of_msmt_runs} for Average of Averages Measurement method...")
        avg_msmt, stdev_pct = sample_power_meter(sample_qty, interval=interval)
        avg_list.append(avg_msmt)
        
        if meter_unit == "dBm":
            sdev_List.append(10 ** (avg_msmt / 10))
        
    avg_msmt = sum(avg_list) / len(avg_list)
    if meter_unit == "dBm":
        stdev = statistics.pstdev(sdev_List)
        avg_msmt_for_sdev = sum(sdev_List) / len(sdev_List)
        stdev_pct = stdev / avg_msmt_for_sdev * 100
    else:
        stdev = statistics.pstdev(avg_list)
        stdev_pct = stdev / avg_msmt * 100
...
def initialize_measurement_system():
    ...
    if debug_flag == False:
        ...
        if pmRangeAuto != "driver cmd not found":
            response = writeVisa(pmeter_resource, pmRangeAuto, opc=False)
    ...
    if debug_flag == False:
        if amplifier == 1:
            temp_msg = '- Ensure the amplifier is powered-OFF before continuing.'
            msg_box_simple(temp_msg)
            ...
            temp_msg = f'- Connect the {step_att_name} channel X to the 11 dB Step Attenuator\n\n' \
                       ...
                       f'- Connect the {generator_name} generator OUTPUT to the amplifier INPUT\n\n' \
                       f'- Connect the amplifier OUTPUT to the {step_att_name} attenuator stack INPUT\n\n' \
                       f'- Connect the {step_att_name} attenuator stack OUTPUT to the DUT INPUT\n\n' \
                       f'- Wait for the measurement process to complete...'
            DisplayImage_pysimplegui("./Images/Basic_Connection_amp.png", text=temp_msg, button_name="Continue", xSize=1600,
                                     ySize=1600)
            
            temp_msg = '- Ensure the amplifier is powered-ON before continuing.'
            msg_box_simple(temp_msg)
    ...
    if debug_flag == False:
        ...
        level_generator_and_power_meter(gen_power, leveling_tol=0.10, settling_time=5, safe=True, max_output=output_limit, ampUsed=amplifier, approxGain=ampGain)
...
def setPMtoLowestPossibleRange(missingCmdFlag="driver cmd not found", successFlag=1, errorFlag=0, overRangeVal=999_999_999):
    printLog("Attempting to set PM to lowest possible range", console=False)
    if (pmRangeSet != missingCmdFlag):
    
        pm_val = overRangeVal
        for rangeVal in range(10):
            response = writeVisa(pmeter_resource, pmRangeSet.replace('<val>', f"{rangeVal}"))
            pm_level, stdev = sample_power_meter(2, interval=0.1, status_bar=False)
         
            if pm_level < overRangeVal:
                break
...
def perform_lin_msmt(steps_list, tol_list, ref_pow, sample_qty, amplifier_used, amp_gain):
    def dut_bias_msmt(sample_qty, reference_pow_setting, gen_pow_level, msmt_interval=1, settle_time=10):
        ...
        temp_str = setPMtoLowestPossibleRange(missingCmdFlag="driver cmd not found", successFlag="Succeeded in setting", errorFlag="Failed to set")
        printLog(f"{temp_str} the PM to the lowest possible range.", console=False)
        pm_level = queryVisa(pmeter_resource, pmRead)
    ...
    def msr_lin_steps(settle_time=5, sample_qty=1):
        sample_qty_if_failed_step = sample_qty * 2
        if debug_flag:
            settle_time = 0.5
            sample_qty = 1
        ...
        columnar_data_list = []
        last_tur = 999
        currently_running_first_step = True
        for index, lin_step in enumerate(steps_list):
            ...
            while loop_step:
                ...
                if loop_step_cntr > 1:
                    ...
                    time.sleep(settle_time)
                    avg_msmt, stdev = sample_power_meter(3, interval=sampling_intv, status_bar=False)
                ...
                # Check to ensure the power meter is in the lowest possible range
                avg_msmt, stdev_pct = sample_power_meter(2, interval=0.1, status_bar=False)
                if avg_msmt > 999_999_999 or currently_running_first_step:
                    temp_str = setPMtoLowestPossibleRange(missingCmdFlag="driver cmd not found", successFlag="Succeeded in setting", errorFlag="Failed to set")
                    printLog(f"{temp_str} the PM to the lowest possible range.", console=False)
                    
                # Clear this flag because the need for it has passed
                if currently_running_first_step:
                    currently_running_first_step = False
                ...
                # Get the average of averages, if the TURs are terrible (indicating abnormally noisy sensor)
                meter_unit = "dBm"
                if last_tur < 2 and remediateTUR_flag == True:
                    qty_of_avgs = 6
                    avg_msmt, stdev_pct = sample_power_meter_avg_of_avgs(sample_qty_current, meter_unit, lin_step, interval=sampling_intv, qty_of_msmt_runs=qty_of_avgs, turValue=last_tur)
                    avg_of_avgs_method = True
                else:
                    avg_msmt, stdev_pct = sample_power_meter(sample_qty_current, interval=sampling_intv)
                    avg_of_avgs_method = False
                ...
                if avg_of_avgs_method == True:
                    sample_qty_current = qty_of_avgs * sample_qty_current        # Update sampling quantity to reflect the number of averaged measurements that were performed
...
def deleteTxtFileLastLine(textFilePath):
...
# zeroSetFailedCalibration, performZsTestAgain = zeroSetCalRoutine(zeroSetFailedCalibration, pmExtZero, calCorrectorDataFileZeroSet, zeroSetToleranceWatts = extZeroSetToleranceWatts, asset = dut_asset, gbMethod = zs_eval_type)
def zeroSetCalRoutine(zeroSetFailedCalibration, range_description, zero_method_CMD, msmt_File_Path, zeroSetToleranceWatts = 0, asset = "Unknown", gbMethod="17025"):

    def resultsToDatFile(dataByLine,resultsDataFile):
        # This function is specifically for creating .dat results files which can be loaded into the
        # PS-Cal corrector program. Applies a newline character to the data automatically

        dataByLine += "\n"

        write_mode = "a"
        file1 = open(resultsDataFile, write_mode)

        file1.writelines(dataByLine)
    ...
    clear()
    printLog("Performing Zero Set Calibration...", console=verbose_flag)
    print(f"Zero Set zeroing method is {range_description}.")  
          
    printLog("Starting zero set calibration routine", console=verbose_flag)
    if zeroSetFailedCalibration == False:
        print("\nTEMPLATE CONFIGURED TO PERFORM ZERO SET CALIBRATION")
    zeroSetFailedCalibration = False

    time.sleep(1)
    print("\nSetting up zero set calibration process...")
    
    # Reset the sensor/power meter
    print(f"Sending command: {pmRst}... >")
    writeVisa(pmeter_resource, pmRst)
    time.sleep(5)
    
    # Set sensor for initial measurement
    print(f"Sending command: {pmConfig}... >")
    writeVisa(pmeter_resource, pmConfig)
    time.sleep(1)
    
    # Make sure the sensor is set to its lowest possible range
    print(f"Sending command to set PM to lowest possible range... >")
    temp_str = setPMtoLowestPossibleRange(missingCmdFlag="driver cmd not found", successFlag="Succeeded in setting", errorFlag="Failed to set")
    printLog(f"{temp_str} the PM to the lowest possible range.", console=False)
    ...
    writeLog("Configuring sensor settings: {}".format(pmConfigZS), logFile)
    
    # input(f"{type(pmConfigZS)} >... ")
    
    tempStr = f"{type(pmConfigZS)}"
    
    if "list" in tempStr:
        # Commands must be sent sequentially
        
        for scpi_cmd in pmConfigZS:
            print(f"Sending command: {scpi_cmd}... >")
            writeVisa(pmeter_resource, scpi_cmd)
            time.sleep(0.1)
        # input("Sent Zero set config command list > ...")
    else:
        print("Zero set config command is NOT list type! :")
        print(f"Sending command: {pmConfigZS}... >")
        writeVisa(pmeter_resource, pmConfigZS)
        time.sleep(0.5)
    
    print(f"Sending command: {pmAutoAvgOn}... >")    
    writeVisa(pmeter_resource, pmAutoAvgOn)
    response = queryVisa(pmeter_resource, pmAvgQuery)
    writeLog("Power meter response to whether automatic averaging is on or off: {}".format(response), logFile)
    
    # Ensure the sensor is not connected to an active RF source
    writeVisa(pmeter_resource, zero_method_CMD)
    writeVisa(pmeter_resource, pmZero)
    print(f"Sending command: {pmZero}... >")
    startSensorZero = time.time()
    writeVisa(pmeter_resource, pmZero)
    time.sleep(5)
    print("\nStabilizing sensor...")
    response = queryVisa(pmeter_resource, pmOpc, timeOutms = 30_000)
    stopSensorZero = time.time()
    sensorZeroingProcessTime = stopSensorZero - startSensorZero
    ...
    while tempBool == True:
        ...
        response = ""
        while not "1" in response:
            time.sleep(1)
            response = queryVisa(pmeter_resource, pmOpc, retryQty=1)
        
        response = queryVisa(pmeter_resource, "FETC?", timeOutms = 30_000)
        printLog(f"Zero Set Power Level Check: {response}", console=verbose_flag)
        response = sanitize_variable(response, specified_class='float', default_response=999_999_999)
        writeLog("Checking that sensor is not connected to live RF. Sensor measured: {} dBm. Cut-off from template is: {} dBm".format(response, appliedPowerFlagLevel), logFile)

        if response > appliedPowerFlagLevel:
            writeLog("Warned user to remove sensor from RF power", logFile)
            msg_box_simple("Ensure the DUT sensor is not connected to any active power source, and terminate the sensor with an appropriate load (if necessary).")
        else:
            tempBool = False

    # Start the calibration routine
    print("\nStarting the ZS calibration routine...\n")
    time.sleep(1)
    print("\nStarting the Zero Set Calibration process...")
    time.sleep(2)

    sensorMsmtTime = 12.5

    # Measurement Loop Process Start -----------------------------------------------------------------------------------
    clear()
    msmtSampleSeries = []
    sensorMsmtTimeList = []
    startSampleLoop = 0
    ...
    testTime3 = 0
    testTime4 = 0
    zeroSetCalibrationStartTime = time.time()
    for sampleNumber in range(zeroSetQtyOfRuns):

        sampleNumberGUI = sampleNumber + 1

        # Re-Zero the sensor
        sensorZeroingProcessTime = 0
        if sampleNumber > 0:
            startSensorZero = time.time()
            time.sleep(1)
            print("\nZeroing the sensor...")
            ...
            time.sleep(1)
            response = queryVisa(pmeter_resource, pmOpc, timeOutms = 30_000)
            print("\nPower meter response to zeroing process: {}".format(response))
            writeLog("Power meter response to zeroing process: {}".format(response), logFile)
            stopSensorZero = time.time()
            sensorZeroingProcessTime = stopSensorZero - startSensorZero
        

        # Samples Acquisition Loop Start -------------------------------------------------------------------------------
        spinner = 0
        msmts = []
        testTime1 = (zeroSetQtyOfRuns * sensorMsmtTime)
        testTime2 = testTime1 * zeroSetQtyOfRuns
        testTime3 = (zeroSetQtyOfRuns * sensorZeroingProcessTime)
        testTime4 = testTime3 - sensorZeroingProcessTime
        testTime = testTime2 + testTime4
        for i in range(zeroSetQtyOfSamples):
            if startSampleLoop != 0 and endSampleLoop != 0:
                sensorMsmtTime = endSampleLoop - startSampleLoop
                sensorMsmtTimeList.append(sensorMsmtTime)
                sensorMsmtTime = statistics.mean(sensorMsmtTimeList)
                testTime1 = (zeroSetQtyOfRuns * sensorMsmtTime)
                testTime2 = testTime1 * zeroSetQtyOfRuns
                testTime3 = (zeroSetQtyOfRuns * sensorZeroingProcessTime)
                testTime4 = testTime3 - sensorZeroingProcessTime
                testTime = testTime2 + testTime4
                writeLog("sensorMsmtTime: {}".format(sensorMsmtTime), logFile)

            startSampleLoop = time.time()

            elapsedTime = (i * sensorMsmtTime) + (sampleNumber * zeroSetQtyOfRuns * sensorMsmtTime) + (sampleNumber * sensorZeroingProcessTime)
            elapsedTime1 = (i * sensorMsmtTime)
            elapsedTime2 = (sampleNumber * zeroSetQtyOfRuns * sensorMsmtTime)
            elapsedTime3 = ((sampleNumber) * sensorZeroingProcessTime)
            elapsedTime = elapsedTime1 + elapsedTime2 + elapsedTime3

            # currentTime = time.time()
            # elapsedTime = currentTime - zeroSetCalibrationStartTime
            remainingTime = (testTime - elapsedTime) / 60

            remainingMinutes = math.floor(remainingTime)
            remainingSeconds = remainingTime - remainingMinutes
            remainingSeconds = round((60 / 100 * remainingSeconds * 100))
            if remainingSeconds < 10:
                remainingSeconds = "0" + str(remainingSeconds)
            ...
            if debug == True:
            # if True:
                print("Test Time: {}".format(testTime))
                print("Elapsed Time: {}".format(elapsedTime))
                print("Difference: {}".format(remainingTime))
                print("---")
                print("i: {}".format(i))
                print("sensorMsmtTime: {}".format(sensorMsmtTime))
                # print("sensorMsmtTimeList: {}".format(sensorMsmtTimeList))
                print("sampleNumber: {}".format(sampleNumber))
                print("zeroSetQtyOfRuns: {}".format(zeroSetQtyOfRuns))
                print("sampleNumber: {}".format(sampleNumber))
                print("sensorZeroingProcessTime: {}".format(sensorZeroingProcessTime))
                print("testTime1: {}".format(testTime1))
                print("testTime2: {}".format(testTime2))
                print("testTime3: {}".format(testTime3))
                print("testTime4: {}".format(testTime4))
                print("elapsedTime1: {}".format(elapsedTime1))
                print("elapsedTime2: {}".format(elapsedTime2))
                print("elapsedTime3: {}".format(elapsedTime3))
                print("---")
            print("Approximate test time remaining: {}:{} (minutes:seconds)".format(remainingMinutes,remainingSeconds))
            print("========================================================")
            print("Capturing Measurement series {} of {}, sample {}".format(sampleNumberGUI,zeroSetQtyOfRuns,(i+1)))
            ...
            while tempBool == True:
                try:
                    ...
                    while not "1" in response:
                        time.sleep(1)
                        response = queryVisa(pmeter_resource, pmOpc, retryQty=1, timeOutms = 30_000)
                    
                    response = queryVisa(pmeter_resource, pmRead)
                        
                    printLog(f"Zero Set Power Level Check: {response}", console=verbose_flag)
                    response = sanitize_variable(response, specified_class='float', default_response=999_999_999)
                    writeLog("PM Response: {}".format(response), logFile)
                    # input(f"Read Power: {response}")
                    # Convert the value to Watts, because the test tolerance is in Watts
                    responseLin = dBm_mW(response) / 1000
                    tempBool = False
                except Exception as e:
                    print(f"Exception: {e}")
                    tempBool = True
            ...
            clear()
            print("Measured {} dBm ({} nW), Tol: {} nW".format(round(response,3), round((responseLin * 1_000_000_000),3), round((zeroSetToleranceWatts * 1_000_000_000),3)))

            writeLog("Sample {}: {} dBm / {} Watts".format((i+1),response,responseLin), logFile)
            msmts.append(responseLin)
            spinner += 1
            endSampleLoop = time.time()
        msd = statistics.mean(msmts)
        msmtSampleSeries.append(msd)



    msd = statistics.mean(msmtSampleSeries)
    # Obtain the standard deviation of the measurement
    if zeroSetQtyOfRuns > 1:
        msdSdev = statistics.stdev(msmtSampleSeries)
        msdSdev = msdSdev * 2
    else:
        msdSdev = 0

    # Calculate guardbanding limits
    ugb1_Limit = zeroSetToleranceWatts - msdSdev
    writeLog("ZS ugb1_Limit: {}".format(ugb1_Limit), logFile)
    ugb2_Limit = zeroSetToleranceWatts + msdSdev
    writeLog("ZS ugb2_Limit: {}".format(ugb2_Limit), logFile)

    # Evaluate the measurement
    if abs(msd) <= ugb1_Limit:
        eval = "Pass"
    elif (abs(msd) > ugb1_Limit) and (abs(msd) <= zeroSetToleranceWatts):
        if gbMethod == "17025":
            eval = "Pass*"
            ...
            eval = "Pass*"
    elif (abs(msd) > zeroSetToleranceWatts) and (abs(msd) <= ugb2_Limit):
        if gbMethod == "17025":
            eval = "Fail*"
        elif gbMethod == "Z540.3":
            eval = "UGB2"
            ...
            eval = "Fail*"
        zeroSetFailedCalibration = True
    else:
        eval = "Fail"
        zeroSetFailedCalibration = True
    writeLog("ZS Eval: {}".format(eval), logFile)

    # Convert all values to nano Watts
    msd = msd * 1_000_000_000
    msdSdev = msdSdev * 1_000_000_000
    zeroSetToleranceWatts = zeroSetToleranceWatts * 1_000_000_000

    ZS_nominal = 0
    ZS_msd = round(msd,3)
    ZS_limits = round(zeroSetToleranceWatts,3)
    ZS_unc = round(msdSdev,1)
    ZS_PF = eval

    psCalCorrectorData = "{},{},{},{},{},{}".format(ZS_nominal, ZS_msd, ZS_limits, ZS_unc, ZS_PF, range_description)
    resultsToDatFile(psCalCorrectorData, msmt_File_Path)
    writeLog("Wrote the following zero set data line to PS-Cal Corrector file > {} <: {}".format(msmt_File_Path,
                                                                                        psCalCorrectorData), logFile)
    print("\nZero Set calibration completed...")
    time.sleep(2)
    
    # ================= Display ZS Test Data Results ========================
    # Setup and display columnar for measurement display
    headers = ['Nominal (nW)', 'Measured (nW)', 'Limit (nW)', 'Uncertainty (nW)', "Evaluation"]
    data = [f'{ZS_nominal:.0f}', f'{ZS_msd:.4f}', f'{ZS_limits:.4f}', f'{ZS_unc:.2f}', f'{ZS_PF}']

    columnar_data_list = []
    console_text_width = 750
    columnar_data_list.append(data)
    table = columnar(columnar_data_list, headers, no_borders=False, min_column_width=20, terminal_width=console_text_width)
    ...
    # Check if the DUT failed a step and allow for a re-test, if desired
    if zeroSetFailedCalibration == True:
        writeLog("DUT failed zero set calibration; asking user if he would like to re-try.", logFile)
        print("\nOverall Zero Set Test Status: Fail")
        # DUT Information Routine
        
        tempBool = yes_no_popup_simple("The DUT failed zero set calibration. Do you want to re-try?")
        
        if tempBool == True:
            writeLog("User chose to re-attempt the zero set calibration.", logFile)
            performZeroSetCal = True
            # Must delete original results data file prior to re-running the test
            # Function below removes the last line (containing the failed data) of the results file
            deleteTxtFileLastLine(msmt_File_Path)
        else:
            writeLog("User chose not to re-attempt the zero set calibration.", logFile)
            performZeroSetCal = False
    else:
        performZeroSetCal = False
        
    return (zeroSetFailedCalibration, performZeroSetCal)
...
program = GuiProgramWindow(f"Power Sensor Linearity Calibration, v. {program_version} - Football Season Is Here Yaaaaay!")
...
# Check the Zero Set flag
perform_zero_set_bool = program.performZS
zs_eval_type = program.zs_eval_type
...
printLog(f"Asset number / identifier of current DUT: {dut_asset}", console=False)
...
# Perform the DUT Zero Set Test, if Applicable
if perform_zero_set_bool:
    printLog("Performing Zero Set Calibration...", console=verbose_flag)
    
    zsFilePath = PS_CalResultsFolder + "/"
    
    check_and_create_path(zsFilePath, autocreate=True)
    cur_dt = get_convert_timestamp()
    calCorrectorDataFileZeroSet = zsFilePath + "/" + dut_asset + " - " + cur_dt + ".zsc"
    
    calCorrectorDataFileZeroSet = standardize_file_path_format(calCorrectorDataFileZeroSet)

    # Check if measurement file exists
    calCorrectorDataFileZeroSet = rename_if_file_exists(calCorrectorDataFileZeroSet)
        
    printLog("Created PS-Cal Corrector zero set cal data file: " + str(calCorrectorDataFileZeroSet), logFile)
    
    if extZeroSetToleranceWatts != 10 and intZeroSetToleranceWatts != 10:   # "10 is the default value when the zero set tolerance cannot be found
        # perform the multirange zero set test
    
        zeroSetFailedCalibration = False
        while performZeroSetCal == True:
               
            zeroSetFailedCalibration, performZsTestAgain = zeroSetCalRoutine(zeroSetFailedCalibration, "External Zero", pmExtZero, calCorrectorDataFileZeroSet, zeroSetToleranceWatts = extZeroSetToleranceWatts, asset = dut_asset, gbMethod = zs_eval_type)
            performZeroSetCal = performZsTestAgain
            
        performZeroSetCal = True
        zeroSetFailedCalibration = False
        while performZeroSetCal == True:
               
            zeroSetFailedCalibration, performZsTestAgain = zeroSetCalRoutine(zeroSetFailedCalibration, "Internal Zero", pmIntZero, calCorrectorDataFileZeroSet, zeroSetToleranceWatts = intZeroSetToleranceWatts, asset = dut_asset, gbMethod = zs_eval_type)
            performZeroSetCal = performZsTestAgain
            
    elif intZeroSetToleranceWatts != 10:
        # Perform single range zero set testing
        performZeroSetCal = True
        zeroSetFailedCalibration = False
        while performZeroSetCal == True:
...
# Perform the DUT Linearity measurement process:
run_again = True
dut_failed = False
while run_again:
    initialize_measurement_system()
    dut_failed = perform_lin_msmt(lin_steps_list, lin_steps_tol, refStepSetting, samplingQuantity, amplifier, ampGain)
