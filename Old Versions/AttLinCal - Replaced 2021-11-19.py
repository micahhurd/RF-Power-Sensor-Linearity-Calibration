...
def get_dut_template_data(template_filepath=''):
    ...
    biasMsmtQty = sanitize_variable(temp_value, default_response=30, specified_class='int',
                                    ...
                                    eval_threshold=1)
    printLog(f"biasMsmtQty - CFG File Value: {temp_value}, Sanitized Value: {biasMsmtQty}", console=False)
    ...
    settlingTime = sanitize_variable(temp_value, default_response=5, specified_class='int',
                                    eval_operation='at least',
                                    eval_threshold=1)
    printLog(f"settlingTime - CFG File Value: {temp_value}, Sanitized Value: {settlingTime}", console=False)
    ...
    samplingQuantity = sanitize_variable(temp_value, default_response=3, specified_class='int',
                                     eval_operation='at least',
                                     eval_threshold=1)
    printLog(f"samplingQuantity - CFG File Value: {temp_value}, Sanitized Value: {samplingQuantity}", console=False)
...
def level_generator_and_power_meter(target_level, leveling_tol=0.02, settling_time=5, safe=True, max_output=20):
    ...
    while abs_delta > leveling_tol:
        ...
        if safe:
            if not sig_gen_set_level > max_output:
                ...
                temp_msg = f'Sig Gen Exceeded max safe output power ({max_output}) during the leveling routine!\n' \
                           f'Please manually adjust the generator before continuing... '
                msg_box_simple(temp_msg)
        else:
            response = writeVisa(generator_resource, sGenPowSet.replace('<val>', f'{sig_gen_set_level}'))
        print(response)
...
def calc_uncertainty(nom_value, type_a_pct, sample_qty, attenuator_unc_pct=0, bias_sdev_pct=0):
    ...
    sum_sq = (attenuator_unc ** 2) + (type_a ** 2) + (bias_sdev**2)
    sum_contributors = attenuator_unc + type_a + bias_sdev
    ...
    log.info(f'Uncertainty before budget lookup: {unc_2s_pct:.4f} %, {unc_2s:.4f} mW')
    ...
    log.info(f'Uncertainty after budget lookup: {new_unc_2s_pct:.4f} %, {new_unc_2s:.4f} mW')

    temp_str = f'Unc_Eval: attenuator_unc: {attenuator_unc:.4f} mW ({(attenuator_unc/sum_contributors*100):.2f}%), ' \
               f'degrees_freedom: {degrees_freedom:.0f}, ' \
               f'coverage_factor: {coverage_factor:.4f}, ' \
               f'type_a: {type_a:.4f} mW ({(type_a/sum_contributors*100):.2f}%), ' \
               f'bias_sdev: {bias_sdev:.4f} mW ({(bias_sdev/sum_contributors*100):.2f}%), ' \
               f'unc_1s: {unc_1s:.4f} mW, ' \
               f'unc_2s: {unc_2s:.4f} mW' \
               f'unc_2s (after lookup): {new_unc_2s:.4f} mW'
...
def build_step_setting_dict(lin_step_list, ref_level):
    ...
    for item in lin_step_list[::-1]:
        if debug_flag:
            print(f'lin_step: {item}, att setting: {index}')
...
def build_step_nominal_dict(step_setting_dict, ref_level):
    ...
    for item in lin_step_list:
        ...
        if debug_flag:
            print(f'lin_step: {item}, att_rel_ref: {att_actual}, unc: {att_unc}, att_set: {att_setting}, att_ref_set: {att_at_ref_setting}')
...
def create_pscalcorr_lin_dat_file(csv_data_path, ps_cal_path):
    ...
    for item in textfile_list:
        ...
        new_line = f'{dBm_nom:.3f},{dut_msd_dBm} ({sign}{pct_of_nom:.3f}%),{tol_pct},{unc_pct:.4f},{pf_flag}'
        if verbose_flag:
            print(new_line)
...
def initialize_measurement_system():
    ...
    if debug_flag == False:
        ...
        while retry_bool:
            check_leveled = True
            printLog("Checking if Sig Gen can achieve max output level...")
            ...
            response = writeVisa(generator_resource, sGenPowSet.replace('<val>', f'{gen_power}'))
            printLog(response, console=verbose_flag)
            ...
            try:
                ...
                if not response == gen_power:
                    check_leveled = False
                    temp_str = f'Cannot set the signal generator to the specified output level!\n\n' \
                               f'Specified level: {gen_power}, Generator Max: {response}\n\n' \
                               f'Do you want to retry?'
                    temp_bool = yes_no_popup_simple(temp_str)

                    if temp_bool == False:
                        temp_str = f"Could not set the generator output level to {gen_power} dBm!"
                        error_and_exit(temp_str)
                else:
                    check_leveled = True
    ...
    if debug_flag == False:
        ...
        tempBool = yes_no_popup_simple("Do you want to Zero/Cal the DUT (not required)")

        if tempBool:
            temp_msg = f'- Connect the DUT sensor to the power meter channel 1/A\n\n' \
                       f'- Connect the sensor to the {pMeterName} Reference Port\n\n' \
                       f'- Wait for the zero/calibration process to complete...'
            msg_box_simple(temp_msg)
            ...
            if debug_flag == False:
                temp_msg = f'- Connect the DUT sensor to the power meter channel 1/A.'
                msg_box_simple(temp_msg)
                printLog("Zeroing the power sensor...")
                response = writeVisa(generator_resource, sGenRst)
                printLog(response, console=verbose_flag)
                response = writeVisa(pmeter_resource, pmRst, opc=True)
                printLog(response, console=verbose_flag)
    ...
    # level_generator_and_power_meter(0)
    # input(':')
...
def perform_lin_msmt(steps_list, tol_list, ref_pow, sample_qty):
    def dut_bias_msmt(sample_qty, reference_pow_setting, gen_pow_level, msmt_interval=1, settle_time=10):
        ...
        avg_msmt, stdev = sample_power_meter(sample_qty, interval=msmt_interval)

        avg_msmt = reference_pow_setting - avg_msmt

        return (avg_msmt, stdev)

    def sample_power_meter(qty, interval=1):
        ...
        printLog(f"Sampling Power Meter {qty} samples", console=verbose_flag)
        ...
        msmt_list = []
        for number in range(sample_qty):
            time.sleep(interval)
            pm_level = queryVisa(pmeter_resource, pmRead)
            # printLog(pm_level)
            pm_level = sanitize_variable(pm_level, specified_class='float', default_response=999_999_999)
    ...
    def att_debug_test_routine(lin_step_to_att_set_dict):
        ...
        while loop:
            try:
                step = input("Enter a desired step attenuator value (\"999\" exits): ")
                step = step.strip()
                step = int(step)
                step = abs(step) * -1
    ...
    def msr_lin_steps(settle_time=5, sample_qty=1):
        ...
        response = writeVisa(generator_resource, sGenPowSet.replace('<val>', f'{steps_list[-1]}'))
        printLog(response, console=verbose_flag)
        response = writeVisa(generator_resource, sGenOn)
        printLog(response, console=verbose_flag)
        ...
        for index, lin_step in enumerate(steps_list):
            ...
            while loop_step:
                ...
                if loop_step_cntr > 1:
                    ...
                    time.sleep(settle_time)
                    avg_msmt, stdev = sample_power_meter(sample_qty)
                ...
                # Allow testing of the attenuator
                test_att_debug = False
                if test_att_debug == True:
                ...
                lin_steps_dev.append(stdev)

                # Convert all uncertainty contributors to %
                type_a_percent = dBtoPercent(stdev)

                bias_stdev_percent = dBtoPercent(bias_stdev)

                att_unc_percent = dBtoPercent(att_unc)

                temp_str = f'Uncertainty Contributors: ' \
                           f'type_a: {stdev:.4f} dB ({type_a_percent:.4f}%), ' \
                           f'bias_stdev: {bias_stdev:.4f} dB ({bias_stdev_percent:.4f}%), ' \
                           f'att_unc: {att_unc:.4f} dB ({att_unc_percent:.4f}%)'
                printLog(temp_str, console=verbose_flag)

                # Calculate measurement uncertainty
                unc_2s_mW = calc_uncertainty(lin_step_mW, type_a_percent, sample_qty, attenuator_unc_pct=att_unc_percent,
                                             bias_sdev_pct=bias_stdev_percent)
                ...
                if fail_bool == True:
                    if loop_step_cntr > 1:
                        ...
                        loop_step = False
                elif (eval_str.lower() == 'ugb1') and (loop_step_cntr < 2):
                    loop_step = True
                else:
                    loop_step = False
                ...
                print(table)
                # header = f'{"Step (dBm),":^9}{"Actual,":9}{"Msd (dB),":9}{"L.L (mW),":9}{"Msd (mW)":9}{"U.L.,":9}{"Unc. (mW)":9},{eval_str},{fail_bool},{percent_tol},{tur}'
                # toDisp = f'{lin_step:^9.0f}{true_att:9.4f} dBm, {avg_msmt_normalized:5.4f} dBm, {lower_lim_mW:5.4f} mW, {avg_msmt_mW:5.4f} mW, {upper_lim_mW:5.4f} mW, {unc_2s_mW:5.4f} mW, {eval_str:5}, {fail_bool:5}, {percent_tol:5.1f} % of Tol., {tur:5.1f}:1 TUR'
                # printLog(header)
                # printLog(toDisp)
    ...
    att_ref_step_setting = int(lin_step_to_att_set_dict[ref_pow])
    bias_msmt, bias_stdev = dut_bias_msmt(biasMsmtQty, true_att, steps_list[-1])
    if verbose_flag:
        print(f'bias_msmt {bias_msmt}, {bias_stdev}')
    if debug_flag:
        input(':::')
...
class GuiProgramWindow:
    ...
    def _thread_function(self):
        ...
        layout = [[self.sg.Menu(menu_def, tearoff=False)],
                  ...
                  [self.sg.Text('DUT Asset Number:'), self.sg.In(key='-dut_asset_in-', **{'size': (7, 20)}, disabled=False, focus=True)],
                  [self.sg.FileBrowse('Browse DUT Cal Template File',target='-dutFile-',file_types=self.DUT_File_Type, initial_folder=self.DutDataFolder)],
                  [self.sg.In(key='-dutFile-', **fb)],
                  [self.sg.Button('Load DUT Cal Template',  key='-load_template_btn-')],
                  [self.sg.Text('_' * lineLength)],
                  [self.sg.Text('Set Cal Standards Data:')],
                  [self.sg.FileBrowse('Browse 11 dB Attenuator Data File', target='-stdFile11-', file_types=self.STD_File_Type, initial_folder=self.StdDataFolder, disabled=True)],
                  ...
                  [self.sg.Button('Confirm Attenuator Resource', **resource_button, key='-att_rsrc_btn-', disabled=True), self.sg.In(key='-att_rsrc-', **resource_field, disabled=True), self.sg.Text('        ', key='-att_check-')],
                  [self.sg.Button('Confirm Generator Resource', **resource_button, key='-gen_rsrc_btn-', disabled=True), self.sg.In(key='-gen_rsrc-', **resource_field, disabled=True),
                   self.sg.Text('        ', key='-gen_check-')],
                  [self.sg.Button('Confirm Power Meter Resource', **resource_button, key='-pm_rsrc_btn-', disabled=True), self.sg.In(key='-pm_rsrc-', **resource_field, disabled=True),
                   self.sg.Text('        ', key='-pm_check-')],
                  [self.sg.Text('_' * lineLength)],
                  [self.sg.Button('Start Linearity Calibration', font=('Helvetica', 10, 'bold'), disabled=True,
                                  key='-StartCal-'), self.sg.Text(
                      '                                                                                      ',
                      key="-TF2-")],
    ...
    def create_att_visa(self):
        ...
        self.set_attenuator_resource = set_visa_resource(self.step_att_name, search_resource_string=self.stepAttVisaResourceIdent,
                                                perform_idn=False)
        self.window['-att_rsrc-'].update(self.set_attenuator_resource)

    def create_gen_visa(self):
        print('Create Generator VISA')
        self.set_generator_resource = set_visa_resource(self.generator_name, search_resource_string=self.sGenVisaResource)
        self.window['-gen_rsrc-'].update(self.set_generator_resource)

    def create_pm_visa(self):
        print('Create PM VISA')
        self.set_pmeter_resource = set_visa_resource(self.pMeterName, search_resource_string=self.pMeterVisaResourceIdent)
    ...
    def Start_Lin_Test(self):
        ...
        try:
            ...
            self.att_data_list11, self.att_data_list110, error_msg = get_attenuator_standard_data(self.att_11_data_filepath,
                                                                         self.att_110_data_filepath)
        ...
        if len(self.dut_asset_number) == 0:
            ...
            msg_box_simple(f"Failed to load the 11 and 110 dB attenuator data files. Error:\n\n{error_msg}\n\nPlease try again!")
            self.window['-stdFile11-'].update('')
            self.window['-stdFile110-'].update('')
    ...
    def about_menu_selection(self):
        temp_str = f'You must be pretty bored to press the \"About\" button. Sorry things aren\'t more exciting right now.' \
                   ...
                   f'If the fact of death bothers you, and you know in the ' \
                   f'back of your mind that you shouldn\'t die, but you know you\'re going to anyway, and that bothers ' \
                   f'you. Well, you\'re right to be bothered; death should bother you because man wasn\'t made to ' \
                   f'die. \"\n\n'
...
cwd = os.getcwd() + "\\"
clear()
userInterfaceHeader(program_name, program_version, cwd, logFile)
print("=======================================================================")
...
program = GuiProgramWindow("Power Sensor Linearity Calibration")
...
# Create a PS-Cal Corrector Compatable linearity data file
...
plot_data(msmt_file_name, plot_file_name, plt_x=plot_x_inches, plt_y=plot_y_inches)
