...
def get_dut_template_data(template_filepath=''):
    ...
    global zeroSetToleranceWatts
    zeroSetToleranceWatts = ""
    # input(f"zeroSetToleranceWatts: {zeroSetToleranceWatts} >...")
    temp_value = readConfigFile(templateFile, "zeroSetToleranceWatts")
    # input(f"zeroSetToleranceWatts, temp_value: {temp_value} >...")
    zeroSetToleranceWatts = sanitize_variable(temp_value, default_response=10, specified_class='float',
                                    eval_operation='at least',
                                    eval_threshold=0)
    # input(f"zeroSetToleranceWatts: {zeroSetToleranceWatts} >...")
    printLog(f"zeroSetToleranceWatts - Template File Value: {temp_value}, Sanitized Value: {zeroSetToleranceWatts}", console=False)
    # global zeroSetToleranceWatts
...
def zeroSetCalRoutine(zeroSetFailedCalibration, zeroSetToleranceWatts = 0, asset = "Unknown", gbMethod="17025"):
    ...
    calCorrectorDataFileZeroSet = zsFilePath + "/" + asset + " - " + cur_dt + ".zsc"
    ...
    # print(f"zeroSetQtyOfRuns: {zeroSetQtyOfRuns} Watts")
    # print(f"Zero Set Tolerance: {zeroSetToleranceWatts} Watts")
    ...
    psCalCorrectorData = "{},{},{},{},{}".format(ZS_nominal, ZS_msd, ZS_limits, ZS_unc, ZS_PF)
    resultsToDatFile(psCalCorrectorData, calCorrectorDataFileZeroSet)
    writeLog("Wrote the following zero set data line to PS-Cal Corrector file > {} <: {}".format(calCorrectorDataFileZeroSet,
    ...
    if zeroSetFailedCalibration == True:
        ...
        if tempBool == True:
            ...
            os.remove(calCorrectorDataFileZeroSet)
        else:
...
if perform_zero_set_bool:
    while performZeroSetCal == True:
        ...
        zeroSetFailedCalibration, performZsTestAgain = zeroSetCalRoutine(zeroSetFailedCalibration = False, zeroSetToleranceWatts = zeroSetToleranceWatts, asset = dut_asset, gbMethod = zs_eval_type)
